import React from "react";
import { BsCurrencyDollar, BsBasketFill } from "react-icons/bs";
import Swal from "sweetalert2";

const ProductDetailsUser = () => {
  async function inputQty() {
    const { value: qty } = await Swal.fire({
      title: "Input the desired quantity",
      input: "number",
      inputLabel: "Product Quantity",
      inputPlaceholder: "Enter qty",
      confirmButtonColor: "#0B4619",
    });

    if (qty) {
      Swal.fire({
        title: `Entered Qty: ${qty}`,
        confirmButtonColor: "#0B4619",
      });
    }
  }

  return (
    <div className="flex h-screen">
      <div className="w-fit bg-white overflow-scroll no-scrollbar">
        <h1 className="text-xl text-center font-semibold pt-10 pb-5 pl-5">
          Product Details
        </h1>
        <div className="max-w-sm rounded overflow-hidden">
          <img
            className="w-full"
            src="data:image/jpeg;base64,"
            alt="Sunset in the mountains"
          />
          <div className="px-6">
            <div className="font-bold text-xl my-2">Organic Mango</div>
            <p className="text-gray-700 text-md my-3">
              lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do
              eiusmod tempor incididunt lorem ipsum dolor sit amet, consectetur
              adipiscing elit, sed do eiusmod tempor incididunt
            </p>
            <div className="flex py-1">
              <BsCurrencyDollar className="mt-1 mr-3" size={20} />
              <p className="text-midColor font-semibold text-md pr-1">5000</p>
              <p>/ 100 gr</p>
            </div>
            <div className="flex py-2">
              <BsBasketFill className="mr-3" size={20} />
              <p className="text-midColor font-semibold text-md pr-1">50</p>
              <p>in stock</p>
            </div>
          </div>
        </div>
      </div>
      <div className="border border-gray-300" />
      <div className="w-full overflow-scroll no-scrollbar bg-white px-5">
        <div>
          <h1 className="text-xl font-semibold pt-10 pb-5 text-center">
            Product Description
          </h1>
          <div>
            <ul>
              <li>
                <h1 className="text-lg font-semibold pb-2">Description</h1>
                <p className="text-md pb-4">
                  Lorem ipsum dolor sit amet, consectetur adipiscing elit.
                  Aliquam sagittis elementum eros, at vestibulum ante. Sed
                  pulvinar justo auctor pulvinar ultricies. Sed ac nibh ac elit
                  mollis porta. Aenean fermentum gravida lorem non rutrum.
                  Curabitur ante enim, ultricies nec nisi vitae, consequat
                  fermentum orci. Donec eu ipsum vel magna elementum dignissim
                  at quis ligula. Integer quis purus consectetur, sagittis
                  tortor eget, vulputate urna. Aenean non consectetur nulla.
                  Duis sed elit sollicitudin orci sagittis varius. Cras
                  condimentum lectus lectus, ac imperdiet tortor ultricies at.
                  Etiam accumsan ligula vel vehicula volutpat. Sed aliquet
                  lobortis tellus, vel blandit sem varius sit amet. Morbi et
                  metus blandit, tristique sem ut, rutrum nisl. Donec nulla
                  nisl, sodales id leo vitae, maximus vehicula dui. Quisque
                  placerat urna at lorem varius sollicitudin vitae eget massa.
                  In egestas, mi in porttitor lacinia, justo sapien faucibus
                  eros, in venenatis purus nibh ac ex. Lorem ipsum dolor sit
                  amet, consectetur adipiscing elit. Aliquam sagittis elementum
                  eros, at vestibulum ante. Sed pulvinar justo auctor pulvinar
                  ultricies. Sed ac nibh ac elit mollis porta. Aenean fermentum
                  gravida lorem non rutrum. Curabitur ante enim, ultricies nec
                  nisi vitae, consequat fermentum orci. Donec eu ipsum vel magna
                  elementum dignissim at quis ligula. Integer quis purus
                  consectetur, sagittis tortor eget, vulputate urna. Aenean non
                  consectetur nulla. Duis sed elit sollicitudin orci sagittis
                  varius. Cras condimentum lectus lectus, ac imperdiet tortor
                  ultricies at. Etiam accumsan ligula vel vehicula volutpat. Sed
                  aliquet lobortis tellus, vel blandit sem varius sit amet.
                  Morbi et metus blandit, tristique sem ut, rutrum nisl. Donec
                  nulla nisl, sodales id leo vitae, maximus vehicula dui.
                  Quisque placerat urna at lorem varius sollicitudin vitae eget
                  massa. In egestas, mi in porttitor lacinia, justo sapien
                  faucibus eros, in venenatis purus nibh ac ex. Lorem ipsum
                  dolor sit amet, consectetur adipiscing elit. Aliquam sagittis
                  elementum eros, at vestibulum ante. Sed pulvinar justo auctor
                  pulvinar ultricies. Sed ac nibh ac elit mollis porta. Aenean
                  fermentum gravida lorem non rutrum. Curabitur ante enim,
                  ultricies nec nisi vitae, consequat fermentum orci. Donec eu
                  ipsum vel magna elementum dignissim at quis ligula. Integer
                  quis purus consectetur, sagittis tortor eget, vulputate urna.
                  Aenean non consectetur nulla. Duis sed elit sollicitudin orci
                  sagittis varius. Cras condimentum lectus lectus, ac imperdiet
                  tortor ultricies at. Etiam accumsan ligula vel vehicula
                  volutpat. Sed aliquet lobortis tellus, vel blandit sem varius
                  sit amet. Morbi et metus blandit, tristique sem ut, rutrum
                  nisl. Donec nulla nisl, sodales id leo vitae, maximus vehicula
                  dui. Quisque placerat urna at lorem varius sollicitudin vitae
                  eget massa. In egestas, mi in porttitor lacinia, justo sapien
                  faucibus eros, in venenatis purus nibh ac ex.
                </p>
              </li>
              <li>
                <h1 className="text-lg font-semibold pb-2">Category</h1>
                <p className="text-md pb-4">Fruits</p>
              </li>
              <li>
                <h1 className="text-lg font-semibold pb-2">Condition</h1>
                <p className="text-md pb-4">Ripe</p>
              </li>
              <li>
                <h1 className="text-lg font-semibold pb-2">Expiry Date</h1>
                <p className="text-md pb-4">30/05/2022</p>
              </li>
            </ul>
          </div>
        </div>
        <div align="center" className="static bottom-0 right-1/3">
          <button
            className="rounded-md p-3 bg-darkColor text-lightColor"
            onClick={inputQty}
          >
            Add to Cart
          </button>
        </div>
      </div>
    </div>
  );
};

export default ProductDetailsUser;
